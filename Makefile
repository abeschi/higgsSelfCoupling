...
ROOTGLIBS     = $(shell root-config --glibs) -lGenVector -lFoam -lMinuit -lTMVA -lMLP -lXMLIO  -lTreePlayer -lRooFit -lRooFitCore -lRooStats
...
SONAME	 =  libhiggsSelfCoupling.so
SOFLAGS  =  -Wl,-soname,
...
SONAME	 =  libhiggsSelfCoupling.dylib
...
$(BIN)%$(BINSuf): $(PRG)%$(PRGSuf) $(HDRS) $(LIB)$(SONAME)
	$(CPP) $< $(CPPFLAGS) $(GLIBS) -L$(LIB) -lhiggsSelfCoupling -o $@
